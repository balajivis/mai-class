...
## Three rounds, one protocol

### Round 1 — Free-for-all (2 min)
- **No CLI.** Edit `tasks.md` (a markdown copy of the backlog) directly to record claims and outputs.
- Append-only discipline like Lab 1.
- This will go badly. That's the point — it sets up Round 2.
...
- **Read before claim.** Run `./task-cli/task list --open` (or read `tasks.md` in Round 1) every loop.
...
- **Round 2:** which agent finished the most? Who got the easy tasks? Were any tasks orphaned? The fastest agent cherry-picked.
- **Round 3:** were tasks orphaned because no agent had the capability? Did anyone claim outside their capability and produce weak output? How is contract-net different from "first-claim with politeness"?
