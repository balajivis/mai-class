...
2. Read `PRINCIPLES.md` — understand the product constraints your research should respect
...
**Find something that needs follow-on work?** Add to `backlog.md` under `## Inbox`:
```
- [ ] [follow-on task] — Researcher — [timestamp]
```

**For anything substantive**, write a detailed entry file to `entries/`:
```
