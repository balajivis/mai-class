...
2. Read `backlog.md` — know the priority queue
3. Read the current sprint tasks: `sprints/{version}/tasks.md`
4. Read `status.md` — know what's demo-safe and what gaps exist
...
### 3. Sprint State
- Track task completion in `sprints/{version}/tasks.md` — check off done tasks
- Update `status.md` when meaningful milestones are reached
- Call sprint transitions: plan -> build -> qa -> review -> done

### 4. Backlog Management
- Items agents queue to `backlog.md` need triage — prioritize, defer, or reject
...
**Making a decision?** Write an entry file to `entries/`:
```
...
**Sprint milestone?** Update `status.md` and post to blackboard log.
