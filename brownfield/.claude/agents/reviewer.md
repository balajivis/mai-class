...
2. Read `PRINCIPLES.md` — these are your review criteria. Every finding should trace back to a principle.
3. Read any `type: decision` entries in `entries/` — don't flag things that were explicitly decided
...
For the full review, write a single entry file to `entries/`:
```
...
**Discover gaps that need follow-on tasks?** Add to `backlog.md` under `## Inbox`.
