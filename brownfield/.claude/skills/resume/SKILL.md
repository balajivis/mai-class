...
You are giving a start-of-session briefing. Your job: get the human back to full context in 2 minutes, then hand off to whatever they want to work on.

## What You're Solving

The human has been away — hours, days, maybe a week. They've lost the thinking context. Code artifacts tell them what was built, but not:
...
### Blackboard
- `board.md` — current state (blockers, decisions, queued items)

### Latest checkpoint
- Find the most recent file in `entries/` with type `checkpoint`
...
### Sprint state
- Find the latest sprint in `sprints/`
- Read its `tasks.md` — how many done vs remaining?
- Skim `prd.md` for the sprint goal

### Archives (if stale)
- If the latest checkpoint is >3 days old, also read the last 3 files in `archives/` to fill in the gap
