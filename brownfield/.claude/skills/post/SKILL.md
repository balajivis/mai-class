...
Create `entries/{datestamp}-{role}-{type}-{slug}.md`:
...
For the role: if the human is posting (you're running in their terminal), use `Human`. If an agent is posting (Dev, PM, Test), use that role name.

The `{slug}` is 2-4 words from the message, hyphenated, lowercase.

## Update the Right File

**For finding, steer, decision, blocker, available, handoff** — append a bullet to the correct section in `board.md`:
...
**For queue** — append to `backlog.md` under `## Inbox`:
```
