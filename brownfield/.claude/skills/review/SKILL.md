---
name: review
description: "Rate an agent's work. Records approve/reject/edit decision in decisions.yaml for HITL competence tracking."
...
1. `sprints/*/tasks.md` — find the task description
2. `git log --oneline -10` — see recent commits related to this task
3. `snapshot.yaml` — check which agent was assigned this task
4. `decisions.yaml` — read existing decisions to get the next REV-NNN id
...
Read `decisions.yaml`, then append a new review entry to the `decisions:` array.
...
Write the updated YAML back to `decisions.yaml`. Use the `yaml` package to parse and stringify — do NOT manually construct YAML strings.
...
- If `decisions.yaml` doesn't exist, create it with the schema header from sprints/v1/tasks.md T05
