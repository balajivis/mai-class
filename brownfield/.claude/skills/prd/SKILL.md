...
1. `board.md` — active blockers, open decisions, queued items to consider
2. `backlog.md` — the full backlog (## Inbox for quick candidates)
3. `status.md` — what's safe to demo, known gaps, sprint history
4. `sprints/` — scan previous sprint prd.md files to avoid repeating work
...
2. **Read agent profiles** — check `.claude/agents/{name}.md` for each agent's capabilities before assigning.
...
Files: sprints/$ARGUMENTS/prd.md
       sprints/$ARGUMENTS/tasks.md
