...
You are running an end-of-session debrief. This is a **conversation**, not a form — spend 5-10 minutes building shared understanding with the human before writing anything.
...
Also read:
- `board.md` — current blackboard state
- The current sprint's `tasks.md` (find the latest sprint in `sprints/`)
- Recent blackboard entries: `entries/` (last 5-10 files)
...
Write to `entries/{date}-{time}-checkpoint.md`:
...
Rewrite `board.md` to reflect current state:
...
- Move removed items to `archives/{date}.md` as a summary
