...
1. Read `board.md` — check for blockers or decisions that affect your work
2. Read `sprints/$ARGUMENTS/tasks.md` — find the **first unchecked task**
3. Read `sprints/$ARGUMENTS/prd.md` — understand the sprint goal
4. Post agent init to board.md:
   - Get timestamp: `date "+%b %-d %-I%p" | tr '[:upper:]' '[:lower:]'`
   - Also create entry file: `entries/{datestamp}-dev-available-v{N}-{task}.md`
     ```yaml
     ---
     type: available
     role: Dev
...
2. Write a blocker entry: `entries/{datestamp}-dev-blocker-{slug}.md`
...
- Entry files go in `entries/{YYYY-MM-DD-HHMM}-{role}-{type}-{slug}.md`
